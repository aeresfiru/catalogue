...
public class OauthClientHttpRequestInterceptor implements ClientHttpRequestInterceptor {
    ...
    public ClientHttpResponse intercept(HttpRequest request, byte[] body, ClientHttpRequestExecution execution)
        ...
        if (!request.getHeaders().containsKey(HttpHeaders.AUTHORIZATION)) {
            var authorizedClient = buildClient();
            var authorization = this.authorizedClientManager.authorize(authorizedClient);
            if (authorization == null) {
                throw new AccessDeniedException("Unauthorized");
            }
            request.getHeaders().setBearerAuth(authorization.getAccessToken().getTokenValue());
        }
        return execution.execute(request, body);
    }

    private OAuth2AuthorizeRequest buildClient() {
