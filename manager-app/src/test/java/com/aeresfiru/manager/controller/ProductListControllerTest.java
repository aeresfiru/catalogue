...
public class ProductListControllerTest {
    ...
    void createProduct_RequestIsValid_ReturnsRedirectionToProductPage() {
        ...
        // when
        var result = this.productController.createProduct(request, model, response, attributes);
    ...
    void getProductsList_ReturnsProductListPage() {
        ...
        doReturn(new PageResponse<>(productList, 0, 10, 2, 1))
                .when(this.productClient).findAllProducts(filter, 0, 10);

        // when
        var result = this.productController.getProductsList(0, 10, filter, model);

        // then
        assertThat(result).isEqualTo("catalogue/products/list");
        assertThat(model.containsAttribute("products")).isTrue();
        verify(this.productClient).findAllProducts(filter, 0, 10);
        verifyNoMoreInteractions(this.productClient);
    }
}
